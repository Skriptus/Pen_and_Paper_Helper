...
func get_user(email):
	...
	emit_signal("got_user",User_doc.fields2dict(User_doc))
...
func get_game(gamename):
	...
	emit_signal("got_game",Game_doc.fields2dict(Game_doc))
...
func update_game(game_dict:Dictionary):
	var Game_doc := FirestoreDocument.new(game_dict,game_dict["Host"],game_dict)
	OpenGames.update(game_dict["Host"],Game_doc.dict2fields(game_dict))
